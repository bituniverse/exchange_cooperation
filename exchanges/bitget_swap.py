...
class bitgetswap(SwapApi,bitget):


    def describe(self):
        return self.deep_extend(super(bitgetswap, self).describe(), {
            'id': 'bitget',
            'name': 'Bitget',
            'countries': ['SG'],
            'version': 'v3',
    ...
    async def fetch_order_book(self, symbol, limit=None, params={}):
        ...
        data = await  self.capiGetMarketDepth(request)
        #
        # spot
        #
        #     {
    ...
    async def fetch_accounts(self, params={}):
        ...
        response =await self.apiGetAccountAccounts()
        #
        #     {
        #         "status":"ok",
        #         "ts":1595679591824,
    ...
    async def fetch_balance(self, params={}):
        ...
        response = await self.swapGetAccountAccounts()
        #
        # spot
        #
        #     {
        ...
        #     ]
        #
        res= self.parse_balance_by_type(type, response)
        res["info"]=response
        return res
    ...
    async def create_order(self, symbol, type, side, amount, price=None,clientOrderId=None, positionSide=None,
        ...
        response =await self.swapPostOrderPlaceOrder(request)
        #
        # spot
        #
        #     {
        ...
        #     }
        #
        res= self.parse_order(response, market)
    ...
    async def fetch_open_orders(self, symbol=None, since=None, limit=None, params={}):
        ...
        query = self.omit(params, 'type')
        data = await self.swapGetOrderOrders(request)
        #
        #  spot
        #
        #
    ...
    async def fetch_orders(self, symbol, since=None, limit=None, fromId=None, direct='next', params={}):
        '''
        合约交易接口 获取订单列表 标注废弃
        :param params:
        :return:
        ...
        if type == 'spot':
            ...
            request['status'] =5  #-1:已撤单（包含风险触发撤销），0:未成交，1:部分成交，2：完全成交， 3:未成交或部分成交，4：已撤单（包含风险触发撤销）或完全成交 5:所有状态
            method = 'swapGetOrderOrders'
        data = await self.swapGetOrderOrders(request)
    ...
    async  def fetch_positions(self, symbol=None, params=None):
        ...
        data =await  self.swapGetPositionAllPosition()
        return self.parse_fetch_positions(data)

    def parse_fetch_positions(self, data):
        if data is None:
    ...
    async def change_leverage(self, symbol, leverage, positionSide=None, params={}):
        ...
        response =await self.swapPostAccountLeverage(request)
        return self.parse_change_leverage(response)

    def parse_change_leverage(self, data):
        if data is None:
            raise NotImplementedError()
        return {"info": data}

    async def change_margin_type(self, symbol, marginType, positionSide=None, params={}):
        '''
        修改用户账户模式 POST
        ...
        response = await  self.swapPostPositionChangeHoldModel(request)
        return self.parse_change_margin_type(response)

    def parse_change_margin_type(self, data):
        if data is None:
            raise NotImplementedError()
        return {"info": data}
    ...
    async def fetch_position_side(self, symbol=None, params=None):
        ...
        response = await self.swapGetPositionSinglePosition(request)
        return self.parse_fetch_position_side(response)


    def parse_fetch_position_side(self,data):
        if data is None:
            raise NotImplementedError()
        p_dict = {}
        p_dict["symbol"] = data["holding"][0]["symbol"]
        p_dict["positionSide"] = "long" if data["holding"][0]["holdSide"] == "1" else "short" #pls check this arg
        p_dict["info"] = data
        return p_dict
    async def change_isolated_margin(self, symbol, positionSide, direction, amount, params=None):
        ...
        response =await  self.swapPostAccountAdjustMargin(request)
        return self.parse_change_isolated_margin(response)

    def parse_change_isolated_margin(self, data):
        if data is None:
            raise NotImplementedError()
        return {"info": data}
    ...
    async def fetch_funding_records(self, symbol=None, since=None, limit=None, fromId=None, direct='next', params=None):
        ...
        response = await  self.swapGetAccountLedger(request)
        return self.parse_fund_record(response)

    def parse_fund_record(self, data):
        if data is None:
            raise NotImplementedError()
        incomes_list = []
        for incomes in data["list"]:
            p_dict = {}
            p_dict["position"] =None
            p_dict["positionValue"] = None
            p_dict["fundingRate"] = None
            p_dict["info"] = incomes
            for key, val in incomes.items():
                if key == "amount":
                    if val==None:
                      ...
                      p_dict["fundingFee"] = float(val)
                elif key == "createTime":
                    p_dict["timestamp"] = int(val)
                elif key == "ledgerId":
                    p_dict["id"] = str(val)
            incomes_list.append(p_dict)
        return incomes_list
    ...
    async def fetch_trading_fee_rates(self, symbol=None, params=None):
        ...
        request = {}
        query = []
        method = 'swapGetAccountUserFee'
        response =await  self.swapGetAccountUserFee()
        return self.parse_fetch_trading_fee_rates(response)

    def parse_fetch_trading_fee_rates(self, data):
        if data is None:
            raise NotImplementedError()
        fee_rate_list=[]
        for fe in data:
            ...
            for k, v in fe.items():
                if k == 'symbol':
                   p_dict["symbol"] = v
                if k == "maker":
                    p_dict["maker"] = float(v)
                if k == "taker":
                    p_dict["taker"] = float(v)
            fee_rate_list.append(p_dict)
