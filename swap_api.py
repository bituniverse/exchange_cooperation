class SwapApi:
    ...
    async def create_order(self, symbol, type, side, amount, price, client_order_id, reduce_only, params=None):
        ...
        :param client_order_id: unique id generated by client side, will be used to check order when the create_order
                                API didn't give back result clearly (eg. timeout when request)
        :param reduce_only:     mark order as close position in dual side mode, or limit order not to open opposite side in
                                single side mode.
        :param params:          dict for non-specific parameters
    ...
    async def fetch_orders(self, symbol, since=None, limit=None, from_id=None, params=None):
        ...
        :param from_id: start id of the orders
        :param params:  dict for non-specific parameters
    ...
    async def fetch_my_trades(self, symbol, since=None, limit=None, from_id=None, params=None):
        ...
        :param from_id: start id of the trades
        :param params:  dict for non-specific parameters
    ...
    async def fetch_positions(self, symbol, params=None):
        ...
        For dual side mode, the value of position should always be positive, and position_side indicates the direction
        ...
        [
            {
                ...
                'open_price':           9291.2,      // average open price
                'mark_price':           9289.9,      // current mark price if has
                'unrealized_profit':    224.2532,    // unrealized profit with current market price or mark price
                'liquidate_price':      8824.33,     // liquidation trigger price
                'leverage':             20,          // current leverage of the position
                'margin_type':          'isolated',   // margin type of the position, 'isolated' or 'crossed'
                'init_margin':          232.28,      // initial margin
                'position_side':        'long',      // side of the position, 'long' or 'short' for dual side mode, 'both' for single side mode
                'info':                 { ... }      // the original response
            },
    ...
    async def change_leverage(self, symbol, position_side, leverage, params=None):
        ...
        :param symbol:          symbol in ccxt standard format, example: 'BTC/USDT'
        :param position_side:   'long' or 'short' for dual side mode, 'both' for single side mode
    ...
    async def fetch_position_side(self, symbol, params=None):
        ...
        example: {
            'symbol':           'BTC/USDT',  // symbol
            'position_side':    'dual',      // 'dual' or 'single'
            'info':             { ... },     // the original response
        }
        """
        raise NotImplementedError()
    ...
    async def change_position_side(self, symbol, position_side, params=None):
        ...
        :param symbol:          symbol in ccxt standard format, example: 'BTC/USDT'
        :param position_side:   'dual' or 'single'
        :param params:          dict for non-specific parameters
        :return:                info
    ...
    async def fetch_funding_records(self, symbol=None, since=None, limit=None, from_id=None, params=None):
        ...
        [
            {
                'id':               '12345-67890:09876/54321',  // string trade id
                'funding_fee':      0.0211432,                  // funding fee
                'position':         0.5,                        // position when funding fee was produced
                'position_value':   4588.22315,                 // value of the position when funding fee was produced, (position * price)
                'funding_rate':     0.0001,                     // funding fee rate
                'timestamp':        1502962946216,              // Unix timestamp in milliseconds
                'info':             { ... }                     // the original response
            }
        ]
    ...
    async def change_isolated_margin(self, symbol, position_side, direction, amount, params=None):
        ...
        :param symbol:          symbol in ccxt standard format, example: 'BTC/USDT'
        :param position_side:   position side for indicate position, 'long' or 'short' for dual side mode, 'both' for single side mode
